...
if [ "$type" == "glob" ]; then
  parallel --joblog "$(basename "$bamdir")"_"$(date "+%b%d%Y_%H%M%S")"_align_stats_joblog.log --progress --bar --verbose --jobs "$cores" align_stats ::: "$(ls "${bamdir%/}"/*/*/align/rep*/*_R1.trim.bam)"
elif [ "$type" == "file" ]; then
  readarray -t raw_bam_list <<<"$bamdir"
  parallel --joblog "align_stats_$(date "+%b%d%Y_%H%M%S")_joblog.log" --progress --bar --verbose --jobs "$cores" align_stats ::: "${raw_bam_list[@]}"
elif [ "$type" == "find" ]; then
  parallel --joblog "$(basename "$bamdir")"_"$(date "+%b%d%Y_%H%M%S")"_align_stats_joblog.log --progress --bar --verbose --jobs "$cores" align_stats ::: "$(find -name "*_R1.trim.bam" "$bamdir")"
else
