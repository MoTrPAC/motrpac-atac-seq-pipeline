...
CROO_OUTPUT_PATH=$3 # gcp path to croo outputs without trailing slash
...
gs_copy() {
  ...
  # merged peak file
  gsutil cp -n "${dir}peak/overlap_reproducibility/overlap.optimal_peak.narrowPeak.gz" "peak/${condition}.overlap.optimal_peak.narrowPeak.gz"
  gsutil cp -n "${dir}peak/overlap_reproducibility/overlap.optimal_peak.narrowPeak.hammock.gz" "peak/${condition}.overlap.optimal_peak.narrowPeak.hammock.gz"

  # pooled signal track
  if [[ $condition != *"GET-STDRef-Set"* ]]; then
    gsutil cp -n "${dir}signal/pooled-rep/basename_prefix.pooled.pval.signal.bigwig" "signal/${condition}.pooled.pval.signal.bigwig"
  fi

  # qc.html
  gsutil cp -n "${dir}qc/qc.html" "qc/${condition}.qc.html"
}
